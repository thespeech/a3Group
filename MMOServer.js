...
function MMOServer() {
	...
	var InterestAreaLoop = function() {
		...
		for(i in ships) {
			for(j in ships) {
				if(((Math.abs(ships[i].x - ships[j].x) < (INTEREST_ZONE+0.5)) &&
				   (Math.abs(ships[i].x - ships[j].x) >= INTEREST_ZONE)) ||
				   ((Math.abs(ships[i].y - ships[j].y) < (INTEREST_ZONE+0.5)) &&
			...
			for(var k in rockets) {
				if(((Math.abs(rockets[k].x - ships[i].x) < (INTEREST_ZONE+0.5)) &&
					(Math.abs(rockets[k].x - ships[i].x) >= INTEREST_ZONE)) ||
				   ((Math.abs(rockets[k].y - ships[i].y) < (INTEREST_ZONE+0.5)) &&
